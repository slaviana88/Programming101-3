...
class GithubSocialNetwork:
    ...
    def __init__(self, username, level):
        if level > 3:
            raise ValueError("Level should be smaller 3")
    ...
    def do_you_follow(self, user):
        return user in self.get_info(self.username)["following"]

    def do_you_follow_indirectly(self, user):
        for followed in self.get_info(self.username)["following"]:
            if user in self.get_info(followed)["following"]:
                return True
    ...
    def who_follows_you_back(self):
        followers = [fr for fr in self.get_info2(self.username)["followers"]]
        following = [fw for fw in self.get_info2(self.username)["following"]]
        result1 = [user for user in followers if user in following]
        result2 = [user for user in followers for followed in following \
                    if user in self.get_info(followed)["following"]]
...
def main():
    username = 'slaviana88'
    network = GithubSocialNetwork(username, 2)
    print(network.get_info(username)['following'])
